...
export function Hero() {
  ...
  return (
    <section className="relative min-h-[100svh] overflow-hidden bg-[#f7f9fc] text-slate-950">
      ...
      <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-7xl items-center px-4 pb-28 pt-28 sm:px-6 lg:px-8">
        <div className="max-w-3xl">
          <div className="mb-6 inline-flex items-center gap-2 text-sm font-bold text-slate-700">
            <Truck className="h-4 w-4 text-brand-blue" />
            Buy, sell and deliver across India
          </div>
          <h1 className="max-w-3xl text-balance text-5xl font-black leading-[1.02] text-slate-950 sm:text-6xl lg:text-7xl">
            The right car should feel right from day one.
          </h1>
          <p className="mt-6 max-w-2xl text-pretty text-base leading-7 text-slate-700 sm:text-lg">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            ...
            <Button asChild size="lg" variant="outline" className="h-12 rounded-sm border-slate-300 bg-white/85 px-6 font-bold text-slate-950 backdrop-blur-sm hover:bg-white">
              <a href="#sell"><Car className="h-4 w-4" /> Sell your car</a>
            </Button>
            <Button asChild size="lg" variant="ghost" className="h-12 rounded-sm px-4 font-bold text-slate-900 hover:bg-white/70 hover:text-slate-950">
              <a href={whatsapp} target="_blank" rel="noreferrer"><MessageCircle className="h-4 w-4 text-brand-red" /> WhatsApp</a>
            </Button>
          </div>

          <div className="mt-8 flex max-w-2xl flex-wrap border-y border-slate-300/70 bg-white/72 backdrop-blur-sm">
            {["200+ point checks", "Transparent pricing", "Pan India support"].map((item) => (
              <div key={item} className="flex min-h-14 flex-1 basis-44 items-center gap-2 px-4 text-sm font-semibold text-slate-700">
      ...
      <div className="absolute bottom-5 left-4 right-4 z-20 mx-auto flex max-w-7xl items-end justify-between gap-4 sm:left-6 sm:right-6 lg:left-8 lg:right-8">
        <div className="flex gap-5">
          {heroScenes.map((scene, index) => (
            <button key={scene.label} type="button" onClick={() => setActive(index)} className={`min-h-11 border-b-2 pb-1 text-sm font-bold transition ${active === index ? "border-brand-red text-slate-950" : "border-transparent text-slate-600 hover:text-slate-950"}`} aria-pressed={active === index}>
              {scene.label}
            </button>
          ))}
        </div>
        <a href="/rk-story" className="hidden min-h-11 items-center gap-2 text-sm font-bold text-slate-700 transition hover:text-brand-blue sm:flex">Read the RK story <ArrowRight className="h-4 w-4" /></a>
      </div>
    </section>
  );
}
